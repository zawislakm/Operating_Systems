...
void count(MemoryBlocks *mem_blocks, char *open_filename) {
    ...
    if (result != 0){
        printf("%s","Błąd przy tworzeniu folderu\n");
        exit(1);
    }
    ...
    int result2 = system(command);
    if (result2 != 0 ){
        printf("%s","Błąd przy liczeniu lini");
        exit(1);
    }
    int lines_count = count_lines_in_wc_output_file(filename);
    sprintf(command, "rm -f %s", filename); // poprawka - brakujący format dla drugiego argumentu
    int result3 = system(command);
    if (result3 != 0 ){
        printf("%s","Błąd przy usuwaniu folderu");
        exit(1);
    }
